...
# 🌲 MIT 6.042J 离散数学：树、生成树与最优路径

| 字段 | 内容 |
|------|------|
| 📅 学习日期 | 2026-03-10 |
| 📖 课程进度 | Lecture 8 - Trees & Minimum Spanning Trees (MST) |
| 🔑 核心关键词 | 无环 (Acyclic)、剪枝 (Pruning)、贪心算法 (Greedy)、交换引理 (Swap Lemma) |

---

## 🛤️ 一、基础：从通路到路径 (Walks vs. Paths)

在复杂的图结构中，我们首先定义如何"移动"。

- **通路 (Walk)**：顶点与边的交替序列。
- **路径 (Path)**：所有顶点都不重复的通路（没有回头路）。

> **引理 1 (Well-ordering Principle 应用)**：如果两个节点间存在通路，则一定存在一条路径。

**证明逻辑**：在所有可能的通路中，必然存在一条"最短通路"。如果这条最短通路有重复节点，我们可以切掉中间的环，使其更短，从而产生矛盾。

---

## 🌲 二、树的数学属性 (Properties of Trees)

树是图论中最完美的结构，它是**连通性**与**精简性**的平衡点。

### 1. 定义

一个图被称为"树"，必须满足：

- **连通 (Connected)**：任意两点皆可达。
- **无环 (Acyclic)**：不存在闭合回路。
...
$$\text{它是树} \iff \text{它恰好有 } n-1 \text{ 条边}$$

> **证明感悟**：利用叶子节点 (Leaf) 进行归纳。每去掉一个度数为 1 的叶子，顶点减 1，边也减 1，结构依然是树。这就像是在做**减法优化**。

---

## ✂️ 三、生成树：在复杂中寻找骨架 (Spanning Trees)

生成树是包含原图所有顶点，但边数被精简到极致（$n-1$ 条）且保持连通的子图。

### 构造逻辑（破坏性创建）

1. 如果一个连通图有环，就删掉环上的一条边。
2. 重复此过程，直到没有环。

> **现实映射**：在 HEOR (卫生经济学) 中，这代表了**"最低限度的覆盖"**。比如在医疗资源匮乏地区，如何用最少的巡回医疗路径连接所有村庄。

---

## 💰 四、最小生成树 (MST) 与贪心策略

当边带有了"权重"（成本/风险/距离）时，我们的目标变成了寻找**总权重最小**的生成树。
...
1. 将所有边按权重从小到大排序
2. 依次选择权重最小的边
3. 约束：如果加入这条边会形成环，则放弃
4. 重复直到连接所有节点
```

### 2. 交换引理 (The Swap Lemma) —— 算法的灵魂

> 为什么**局部最优**能推导到**全局最优**？

假设你选了一条边 $e$，而完美的 MST（叫它 $T^*$）没选它：

1. 如果你强行把 $e$ 放进 $T^*$，必然会产生一个**环**。
2. 在这个环上，必然存在另一条边 $e'$。
3. 根据贪心法则：$\text{weight}(e) \le \text{weight}(e')$
4. **交换操作**：用 $e$ 替换掉 $e'$。
5. 你会得到一个总权重**更小或相等**的生成树。
...
在进行 **Cost-Effectiveness Analysis (成本效果分析)** 时，我们面对的是无数可能的诊疗路径。

- **策略**：通过移除那些"成本高、收益低"的环路（冗余检查），我们将医疗流程简化为一棵"最小生成树"。
- **目标**：以 $n-1$ 的代价（最低成本）覆盖 $n$ 的临床需求。

### 2. 个人感悟：精细化的本质

正如我从去年 9 月至今的转变，从"广撒网"到"锁定高桥教授"：

| 阶段 | 图结构描述 |
|------|-----------|
| 去年 9 月 | 充斥着多余边的"稠密图"，能量在各种无效的环（BG 互卷、彩票校）中耗散 |
| 现在 | 执行了最小生成树算法，剪掉不必要的边，保留连接目标（PhD 项目）的最优路径 |

> 💡 **心得**："精细化"就是一种自动运行的**剪枝算法**。
...
### 1. 树的本质 = 零冗余 (Zero Redundancy)
- 规则：$V$ 个点，$V-1$ 条边。
- 逻辑：多一条边就是浪费钱（环），少一条边就是断了线。

### 2. 最小生成树 (MST) = HEOR 的资源优化
- 场景：用最少的医保基金，连接最多的社区卫生点。
- 策略：贪心算法。永远选最便宜的，只要它不造出“逻辑闭环”。

### 3. 交换引理 (Swap Lemma) = 决策的信心
- 证明：局部最优 = 全局最优。
- 启示：在成本导向的系统中（如物流、基建、基础数学），“只看眼前利益”反而是最高级的智慧。



### 核心定义：路径 (Path) vs 通路 (Walk)
- **Path**: $v_0 \to v_1 \to ... \to v_k$（顶点不重复）。
- **感悟**: 所谓“精细化”，就是把充满回头路的 Walk，优化成直击目标的 Path。

### 树的圣经 (Properties of Trees)
- **定理**: $n$ 个节点，必然只有 $n-1$ 条边。
- **意义**: 在 HEOR 资源分配中，树代表了“连通所有点”的最经济结构。

### 最小生成树 (MST) 的“贪心”证明
- **交换引理 (Swap Lemma)**：
  - 逻辑：如果你不选最便宜的 $e$，加入它必然会成环。
  - 操作：删掉环里那个贵的，换成便宜的。
  - 结论：局部最优（Greedy）在树的构造中可以推导出全局最优。

> **PhD 碎碎念**：
> 这个“交换引理”其实是在告诉我们：**在成本敏感的系统中，犹豫（不选最便宜的）就是犯罪。**

### ⚖️ 贪心算法的“适用边界”：Lec 7 vs. Lec 8

| 维度 | Matching (婚恋匹配) | MST (最小生成树) |
| :--- | :--- | :--- |
| **驱动力** | 个体偏好 (Preferences) | 外部权重 (Fixed Weights) |
| **冲突模式** | 节点竞争：抢到一个，丢掉一片。 | 边权重累加：只要不成环，多多益善。 |
| **贪心结局** | **失败**：产生流氓伴侣，系统崩溃。 | **成功**：交换引理确保了全局最优。 |
| **HEOR 映射** | 医患匹配（必须考虑双方意愿）。 | 医保网络铺设（只考虑建设成本）。 |

**💡 深刻总结：**
- **Matching** 需要的是“妥协”和“延迟满足”（Gale-Shapley 算法中女生说 Maybe）。
- **Trees** 需要的是“果断”和“见钱眼开”（看到便宜的边立刻拿下）。

## 📝 待办与延伸 (To-Do)

- [ ] **代码练习**：在 Python 中调用 `networkx` 库实现 Prim 或 Kruskal 算法（MST 的两种实现）。
- [ ] **深度思考**：如果权重是负数，MST 算法还成立吗？
- [ ] **HEOR 建模**：尝试将一个简单的疾病进展模型抽象为一棵有向树。
...
定理 9-1：无向树的等价定义 3/23/2026
设 $G = \langle V, E \rangle$ 是一个 $n$ 阶 $m$ 条边的无向图。以下命题是等价的：
$G$ 是树（即 $G$ 连通且无回路）。
$G$ 中任意两个顶点之间存在唯一的一条路径。
$G$ 中无回路且 $m = n - 1$。
$G$ 是连通的且 $m = n - 1$。
$G$ 是连通的，且 $G$ 中的任何一条边均为桥。
$G$ 中没有回路，但在任何两个不同的顶点之间加一条新边，所得图中得到唯一的一个含新边的回路。
---
证明过程
为了证明等价性，我们采用循环证明的思路：
$$
...
连通性：根据树的定义，$G$ 是连通的，所以任意两点间至少存在一条路径。
唯一性：反证法。若 $u, v$ 间存在两条不同的路径 $P_1$ 和 $P_2$，那么 $P_1 \cup P_2$ 中必然包含一个回路。这与树"无回路"的定义矛盾。故路径唯一。
---
$(2) \implies (3)$：路径唯一 $\implies$ 无回路且 $m = n - 1$
无回路：若存在回路，则回路上任意两点间至少有两条路径，与唯一性矛盾。
边数公式：利用数学归纳法证明 $m = n - 1$。
当 $n = 1$ 时，$m = 0$，命题成立。
假设对所有 $k < n$ 阶图命题成立。在 $n$ 阶树中删去一个度为 1 的顶点（叶子节点）及其关联边，得到 $n - 1$ 阶子图。由归纳假设该子图有 $n - 2$ 条边，加上刚才删去的 1 条边，总边数 $m = (n-2) + 1 = n - 1$。
---
$(3) \implies (4)$：无回路且 $m = n - 1 \implies$ 连通
设 $G$ 有 $k$ 个连通分支 $G_1, G_2, \dots, G_k$。
由于 $G$ 无回路，每个分支都是一棵树。设第 $i$ 个分支的顶点数为 $n_i$，边数为 $m_i$。
根据树的边数性质，$m_i = n_i - 1$。
总边数：
$$
m = \sum m_i = \sum (n_i - 1) = \left(\sum n_i\right) - k = n - k
$$
已知 $m = n - 1$，故 $n - k = n - 1 \implies k = 1$。所以 $G$ 是连通的。
---
$(4) \implies (5)$：连通且 $m = n - 1 \implies$ 边均为桥
桥的定义：删去该边后图的连通分支数增加。
反证法：若存在边 $e$ 不是桥，则 $G - e$ 仍然连通。
一个 $n$ 阶连通图至少需要 $n - 1$ 条边。但 $G - e$ 只有 $(n-1) - 1 = n - 2$ 条边，无法支撑 $n$ 个顶点的连通性，产生矛盾。
故每条边必须是桥。
---
$(5) \implies (6)$：连通且均为桥 $\implies$ 无回路且加边生唯一圈
无回路：若存在回路，则回路上任意一条边删去后图依然连通，该边不是桥，与条件矛盾。
加边生圈：由于 $G$ 已连通，$u, v$ 间已存在路径 $P$。加入新边 $(u, v)$ 必然与 $P$ 构成一个回路。
唯一性：因为原图中两点间路径唯一（由无回路 + 连通推导），所以加边后形成的回路也是唯一的。
---
$(6) \implies (1)$：无回路且加边生唯一圈 $\implies$ 树
已知 $G$ 无回路。
证明连通：反证法。若 $G$ 不连通，取属于不同连通分支的顶点 $u, v$。在 $u, v$ 间加边不会产生回路（因为原图中两点间没有通路）。这与"加新边必得回路"的条件矛盾。
故 $G$ 必连通。连通且无回路 $\implies$ $G$ 是树。$\blacksquare$

---

> *"A tree is the most efficient way to connect a world without creating confusion."*
