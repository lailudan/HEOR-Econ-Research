...
- 在 HEOR 建模中，介值定理其实是**平衡点（Equilibrium）
- **存在的基石：剂量-收益模型：假设你正在研究某种药物（如息宁）的剂量对睡眠质量的影响。
- 如果剂量 $x_1$ 导致睡眠不足（负向结果），而剂量 $x_2$ 导致副作用过大（正向结果），只要这个生理反应函数是连续的，那么介值定理就保证了：一定存在一个“黄金剂量” $c$，能刚好达到你预设的理想睡眠指标 $N$。
- PhD 级别的思考：在经济学中，我们经常无法精确算出最优解（就像无法手动解开高次方程），但我们可以利用 IVT 证明：只要政策是连续调整的，最优解就一定存在于某个区间内。 这种“存在性证明”是所有高阶经济建模的第一步。

## 3/13 错题复盘：介值定理的多重判定

### ❌ 错误直觉
认为函数只要经过一次目标值，后续区间就不再“必须”含有解。

### ✅ 正确逻辑 (IVT)
只要区间 $[a, b]$ 满足 $f(a) < k < f(b)$ 或 $f(b) < k < f(a)$，该区间就 **必须** 至少有一个解。函数可以多次穿越同一个高度。
...
<img width="1362" height="914" alt="4bc52818-05fe-43e4-ac78-91660ba98576" src="https://github.com/user-attachments/assets/c9a87fe4-9367-4699-a3a8-36b40f755b59" />
