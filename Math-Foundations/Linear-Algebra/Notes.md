...
> **学习者**：Ludan  
> **目标**：为 PhD 阶段的计量经济与 HEOR 建模建立三维空间直觉。

---

## 1️⃣ 基石：点积与余弦定理 (Dot Product)
**核心公式**：$\vec{a} \cdot \vec{b} = ||\vec{a}|| ||\vec{b}|| \cos \theta$

### 证明逻辑：余弦定理法
1. **构建三角形**：由向量 $\vec{a}, \vec{b}$ 和 $\vec{a}-\vec{b}$ 构成。
2. **两套展开对比**：
   - **几何展开**：$||\vec{a} - \vec{b}||^2 = ||\vec{a}||^2 + ||\vec{b}||^2 - 2||\vec{a}|| ||\vec{b}|| \cos \theta$
   - **代数展开**：$||\vec{a} - \vec{b}||^2 = (\vec{a}-\vec{b}) \cdot (\vec{a}-\vec{b}) = ||\vec{a}||^2 + ||\vec{b}||^2 - 2(\vec{a} \cdot \vec{b})$
3. **结论**：对比可知 $-2(\vec{a} \cdot \vec{b}) = -2||\vec{a}|| ||\vec{b}|| \cos \theta$，得证。

---

## 2️⃣ 进阶：叉积模长与正弦关系 (Cross Product)
**核心公式**：$||\vec{a} \times \vec{b}|| = ||\vec{a}|| ||\vec{b}|| \sin \theta$

### 证明核心：拉格朗日恒等式 (Lagrange's Identity)
我们要证明：$||\vec{a} \times \vec{b}||^2 = ||\vec{a}||^2 ||\vec{b}||^2 - (\vec{a} \cdot \vec{b})^2$

#### 代数“消消乐”过程
- **左军 ($||\vec{a}||^2 ||\vec{b}||^2$)**：展开产生 9 个平方项（$a_i^2 b_j^2$）。
- **右军 ($(\vec{a} \cdot \vec{b})^2$)**：展开产生 3 个平方项（$a_i^2 b_i^2$）和 6 个交叉项（$2a_i b_i a_j b_j$）。
- **结果**：相减后，3 个平方项抵消，剩下的 6 个混合项正好等于叉积展开式的模平方。
...
- **本质**：与列向量 $\begin{bmatrix} a_x \\ a_y \\ a_z \end{bmatrix}$ 完全等价，是向量的“代数外衣”。

### 棋盘符号法则（行列式展开）
在计算 $\vec{b} \times \vec{c}$ 时：
$$\vec{b} \times \vec{c} = \hat{i}(b_y c_z - b_z c_y) \color{red}{-} \hat{j}(b_x c_z - b_z c_x) \color{red}{+} \hat{k}(b_x c_y - b_y c_x)$$
- **注意**：$\hat{j}$ 前面是**减号**，这是由行列式位置决定的（正负交替）。
