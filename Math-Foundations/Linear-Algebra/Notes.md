...
## 2. 策略第一步：确立第一个主元 (Pivot 1)

**问题**：现在的左上角是 `5`。如果用 `5` 去消下面的数，会产生很多分数（比如 $1/5$），计算会很痛苦。
**观察**：第二行开头刚好是 `1`！
**操作**：**交换第一行和第二行 ($R_1 \leftrightarrow R_2$)**。
...
### 3.1 消灭第二行的 `5`
**目标**：把第二行开头的 `5` 变成 `0`。
**公式**：$R_2 = R_2 + (-5) \times R_1$ （也就是说：第二行 = 原第二行 - 5倍的第一行）。

**一行行算给你看 (Scratchpad)**：
* **第1列**：$5 + (-5 \times 1) = 0$  ✅ (目标达成)
* **第2列**：$-2 + (-5 \times -1) = -2 + 5 = 3$
* **第3列**：$6 + (-5 \times 3) = 6 - 15 = -9$
* **第4列($b$)**：$0 + (-5 \times 3) = 0 - 15 = -15$
...
### 3.2 消灭第三行的 `-4`
**目标**：把第三行开头的 `-4` 变成 `0`。
**公式**：$R_3 = R_3 + (4) \times R_1$ （利用第一行的 `1`，乘以 `4` 加下来）。

**一行行算出 (Scratchpad)**：
* **第1列**：$-4 + (4 \times 1) = 0$ ✅
* **第2列**：$2 + (4 \times -1) = 2 - 4 = -2$
* **第3列**：$-1 + (4 \times 3) = -1 + 12 = 11$
* **第4列($b$)**：$13 + (4 \times 3) = 13 + 12 = 25$
...
## 4. 策略第二步：确立第二个主元 (Pivot 2)

**问题**：现在我们要处理中间那一列。目前第二行中间是 `3`，第三行中间是 `-2`。
**观察**：第二行全是 `3` 的倍数 ($3, -9, -15$)！
**操作**：**把第二行全体除以 3 ($R_2 \div 3$)**。这能让数字瞬间变小，方便计算。
...
**目标**：利用第二行的新主元 `1`，去消灭第三行的 `-2`。
**公式**：$R_3 = R_3 + (2) \times R_2$ （因为 $-2 + 2 = 0$）。

**一行行算给你看 (Scratchpad)**：
* **第1列**：$0 + (2 \times 0) = 0$ (保持不变，安全)
* **第2列**：$-2 + (2 \times 1) = 0$ ✅ (目标达成)
* **第3列**：$11 + (2 \times -3) = 11 - 6 = 5$
* **第4列($b$)**：$25 + (2 \times -5) = 25 - 10 = 15$

**结果矩阵 (Row Echelon Form)**：
$$
\left[
\begin{array}{ccc|c}
1 & -1 & 3 & 3 \\
...
## 6. 收尾：回代求解 (Back Substitution)

现在矩阵已经变成了阶梯形（上三角），我们可以直接读出答案了。

**Step 1: 解出 Z**
看最后一行：$0x + 0y + 5z = 15$
$$5z = 15 \implies \mathbf{z = 3}$$

**Step 2: 解出 Y**
看第二行：$0x + 1y - 3z = -5$
$$y - 3(3) = -5$$
$$y - 9 = -5 \implies \mathbf{y = 4}$$

**Step 3: 解出 X**
看第一行：$1x - 1y + 3z = 3$
$$x - 4 + 3(3) = 3$$
$$x - 4 + 9 = 3$$
$$x + 5 = 3 \implies \mathbf{x = -2}$$
